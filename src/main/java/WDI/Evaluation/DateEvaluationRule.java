 ...
 * 
 */
public class DateEvaluationRule extends EvaluationRule<Book, Attribute> {

	@Override
	public boolean isEqual(Book record1, Book record2, Attribute schemaElement) {
		if(record1.getPublished_date()==null && record2.getPublished_date()==null)
			return true;
		else if(record1.getPublished_date()==null ^ record2.getPublished_date()==null)
			return false;
		else
			return record1.getPublished_date().getYear() == record2.getPublished_date().getYear();
	}
